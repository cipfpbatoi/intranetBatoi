...
class ColaboracionController extends IntranetController
{
    ...
    public function show($id)
    {
        ...
        $contactCol->concat(Activity::where('model_class','Intranet\Entities\Fct')->where('action','email')->
        whereIn('model_id',hazArray($fcts,'id','id'))->get());
        dd(Activity::where('model_class','Intranet\Entities\Fct')->where('action','email')->
        whereIn('model_id',hazArray($fcts,'id','id'))->get());
        $alumnos = [];
        foreach ($fcts as $fct)
