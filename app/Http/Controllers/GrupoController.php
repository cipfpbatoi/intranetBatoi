...
class GrupoController extends IntranetController
{
    ...
    protected function iniBotones()
    {
        ...
        foreach ($cursos as $curso) {
            if (($curso->aforo == 0) || ($curso->NAlumnos < $curso->aforo * config('variables.reservaAforo'))){
                $this->panel->setBoton('grid', new BotonImg("alumnocurso.registerGrupo/" . $curso->id, ['text' => __('messages.generic.register') . $curso->titulo, 'img' => 'fa-institution']));
    ...
    /**
     * Restringeix la comprovació FOL de grup als grups de primer curs.
     *
