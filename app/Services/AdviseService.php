...
class AdviseService
{
    ...
    private function addDescriptionToMessage()
    {
        ...
        if (isset($this->element->estado)) {
            $description = getClase($this->element).' '
                .primryKey($this->element).' '
                .trans('models.' . getClase($this->element).'.'
                .$this->element->estado);
        }
        if (isset($this->element->descriptionField)) {
            $descripcion = $this->element->descriptionField;
            $long = str_replace(array("\r\n", "\n", "\r"), ' ', $this->element->$descripcion);
            $description =  mb_substr($long, 0, 50) . ". ";
        }
        if (isset($this->element->estado)) {
        ...
        foreach ($this->getAdvises() as $people) {
                switch ($people) {
                    ...
                    case 'jefeDepartamento' :
                        $this->advise(
                            isset($this->element->Profesor->dni)
                                ?$this->element->Profesor->miJefe
                                :authUser()->miJefe
                        );
                        break;
                    default :
