...
(function () {
    ...
    function bootActividadGrid(retriesLeft) {
        if (initActividadGrid()) {
            return;
        }

        if (retriesLeft <= 0) {
            console.warn('DataTables no està disponible: Actividad/grid.js no s’inicialitza.');
            return;
        }

        window.setTimeout(function () {
            bootActividadGrid(retriesLeft - 1);
        }, RETRY_DELAY_MS);
    }

    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', function () {
            bootActividadGrid(MAX_RETRIES);
        });
        return;
    }
