...
<div class="container" >
    ...
    <ul>
       ...
       <li>La matrícula de les FCTs, es realitzarà després de finalitzar els mòduls teòrics.
           ...
           Haureu d'utilitzar l'enllaç de sota per realitzar la matrícula de l’fct a partir del 27 de juny per als grups de CAE i APSD, i a partir del 15 de juliol per al grup d’Imatge per al diagnòstic.</li>
        <li><<a href="http://ext.matricula.cipfpbatoi.es/{{config('curso.convocatoria')}}/{{$aR->token}}" >http://ext.matricula.cipfpbatoi.es/{{config('curso.convocatoria')}}/{{$aR->token}}</a></li>
