...
<p>El meu nom és {{AuthUser()->fullName}} i sóc el professor-tutor del {{config('auxiliares.tipoEstudio.'.$elemento->ciclo->tipo)}}<strong>{{$elemento->ciclo->literal}}</strong>
    del {{config('contacto.nombre')}}.
    </p><p>Les classes de segon curs acaben a principis de març, i després, els alumnes han de fer <strong>{{$elemento->ciclo->horasFct}} hores</strong> de pràctiques en empreses/organitzacions/entitats/etc, amb l'horari normal de l'empresa (que sol ser 40 hores setmanals)
    ...
    Per tot això, ens agradaria que consideràreu la possibilitat d'acollir les pràctiques d'un dels nostres alumnes entre el 11 de març i el 10 de juny, aproximadament.</p>
