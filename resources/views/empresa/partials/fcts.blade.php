<ul class="messages fct">
    @if  (UserisAllow(config('constants.rol.administrador')))
        ...
        <li>
            <div class="message_date">
                ...
                <h4 class="text-info"><acronym title='{{$fct->Instructor->email}} ({{$fct->Instructor->telefono}})'><i class="fa fa-user user-profile-icon"></i> {{$fct->Instructor->nombre}} ({{$fct->Instructor->dni}})</acronym></h4>
            </div>
            <div class="message_wrapper">
                <h4><a href='/fct/{!!$fct->id!!}/edit'><i class="fa fa-edit"></i></a> <a href='/fct/{!!$fct->id!!}/delete'><i class="fa fa-trash"></i></a><span class='info'>{!! $fct->Alumno->FullName !!}</span></h4>
    ...
    @else
        ...
        <li>
            <div class="message_date">
                <h4 class="text-info"><i class="fa fa-calendar user-profile-icon"></i>{{$fct->desde}} - {{$fct->hasta}} <i class="fa fa-group user-profile-icon"></i> {{$fct->horas}}</h4>
                <h4 class="text-info"><i class="fa fa-user user-profile-icon"></i> {{$fct->instructor}} ({{$fct->dni}})</h4>
