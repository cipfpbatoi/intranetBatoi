...
<div class="modal fade" id="AddColaboration" tabindex="-1" role="dialog" aria-labelledby="AddColaborationTitle"
     aria-hidden="true">
            ...
            <form method="POST" class="agua" action="/colaboracion/create">
                <div class="modal-body">
                    ...
                    <input type='text' id='contacto_id' name='contacto'
                           ...
                           class="form-control">
                    <input type="text" name="telefono" placeholder="{{trans("validation.attributes.telef1")}}"
                           value="{{ old('telefono') }}" class="form-control"/>
                    <input type="text" name="email" placeholder="@lang("validation.attributes.email")"
                           value="{{ old('email') }}" class="form-control"/>
                    <input type="text" name="tutor" placeholer="@lang("validation.attributes.tutor")"
                           class="form-control" value='{{authUser()->nombre}} {{authUser()->apellido1}}'/>
                    <input type="text" name="puestos" placeholder="@lang("validation.attributes.puestos")*"
                           value="{{ old('puestos') }}" class="form-control"/>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                    <input id="submit" class="boton" type="submit"
                           value="@lang("messages.generic.anadir") @lang("models.modelos.Colaboracion") ">
                </div>
            </form>
        </div>
    </div>
