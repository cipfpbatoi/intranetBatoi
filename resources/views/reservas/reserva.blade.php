                ...
                <div class="row col-md-5" style="margin-top: 50px;">
                    <div class="borderedondo" style="background-color: #d4d4d4;margin-left: 30px;padding: 10px">
                            ...
                            <div class="col-md-7">
                                <select id="desde" name="desde" class="noFlotar">
                                    ...
                                    @foreach ($horas as $hora)
                                        <option value="{{$hora->codigo}}">{{$hora->turno}} {{$hora->hora_ini}}
                                            -{{$hora->hora_fin}}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-7">
                                <select id="idProfesor" name="idProfesor" class="noFlotar">
                                    ...
                                    @foreach ($profes as $profe)
                                        <option value="{{$profe->dni}}"
                                                @if ($profe->dni == authUser()->dni) selected @endif >{{$profe->apellido1}}
                                            , {{$profe->nombre}}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div id="periodica" class="form-group row">
                            <label for="dia_fin" class="col-md-5"> Tots els <span id="nom_dia_fin"></span> fins el dia:
                            </label>
                            <div class="col-md-7">
