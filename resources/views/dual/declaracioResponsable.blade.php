...
@section('content')
    ...
    <p style="font-size: 20px;text-align: justify;line-height: 1.5em">L'empresa
        <strong>{{$todos->fct->colaboracion->centro->Empresa->nombre }}</strong> amb cif
        <strong>{{$todos->fct->colaboracion->centro->empresa->cif}}</strong>
        i domicili en {{$todos->fct->colaboracion->centro->empresa->direccion  }}
        de {{$todos->Fct->Colaboracion->Centro->Empresa->localidad}}, que participa en el projecte de FP Dual del cicle
        <strong>{{ $todos->fct->Colaboracion->Ciclo->vliteral }}</strong> de grau
        <strong>{{ $todos->fct->Colaboracion->Ciclo->tipo == 1?'mitja':'superior' }}</strong>
        de la familia profesional
        <strong>{{ ucfirst(mb_strtolower(substr($todos->fct->Colaboracion->Ciclo->Departament->vliteral,12))) }}</strong>
        amb el <strong>{{ config('contacto.nombre') }}</strong> de <strong>{{ config('contacto.poblacion') }}</strong>
        de titularitat pública amb codi <strong>{{ config('contacto.codi') }}.</strong></p>
    <p style="font-size: 20px;text-align: justify;margin-top: 75px;line-height: 1.5em"><strong>Fa la següent Declaració
    ...
    <p style="font-size: 20px;text-align: justify;margin-top: 75px;line-height: 1.5em">
        ...
        d’acord amb la normativa vigent ha elaborat i aplica el corresponent protocol de contingència sobre messures
        COVID,
