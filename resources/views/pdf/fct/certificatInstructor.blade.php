    ...
    @if (!isset($empresa->idioma) || $empresa->idioma == 'ca')
        <div class="page" style="font-size: large;line-height: 2em">
            ...
            <div class="container" style="width:100%">
                <p style="text-indent: 30px;text-align: justify">Que l'empresa <strong> {{$empresa->nombre}} </strong> ,
                    ubicada a {{$empresa->direccion}} de/d'
                    {{$empresa->localidad}}, ha col·laborat en les pràctiques corresponents a la Formació de Centres de
                    Treball (FCT) del {{$todos->Colaboracion->Ciclo->Xtipo}}
                    <strong>{{$todos->Colaboracion->Ciclo->vliteral}}</strong> amb una durada
                    de {{$datosInforme['horas']}} hores realitzades durant el curs lectiu {{curso()}}, i en les que han
                    participat els següents alumnes/as:
                </p>
                @include('pdf.fct.partials.llistaAlumnes')
                <p>Que dins d'aquesta empresa, En/Na/N' <strong>{{$datosInforme['instructor']->nombre}}</strong> amb
                    DNI {{$datosInforme['instructor']->dni}}, ha sigut l'instructor/a de les pràctiques
                    formatives.</p>
            </div>
    ...
    @if (!isset($empresa->idioma) || $empresa->idioma == 'es')
        <div class="page" style="font-size: large;line-height: 2em">
            ...
            <div class="container" style="width:95%">
                <p style="text-indent: 30px;text-align: justify">Que la empresa <strong> {{$empresa->nombre}} </strong>
                    , ubicada en {{$empresa->direccion}} de
                    {{$empresa->localidad}}, ha colaborado en las prácticas correspondientes a la Formación de Centros
                    de Trabajo (FCT) del {{$todos->Colaboracion->Ciclo->Xtipo}}
                    <strong>{{$todos->Colaboracion->Ciclo->cliteral}}</strong> de duración {{$datosInforme['horas']}}
                    horas realizadas durante el curso lectivo {{curso()}}, y en las que han participado los siguientes
                    alumnos/as:
                </p>
                @include('pdf.fct.partials.llistaAlumnes')
                <p>Que dentro de esta empresa, Don/Doña <strong>{{$datosInforme['instructor']->nombre}}</strong> con
                    DNI {{$datosInforme['instructor']->dni}}, ha sido el/la instructor/a de las prácticas
                    formativas.</p>
            </div>
        ...
        @if (!isset($empresa->idioma) || $empresa->idioma == 'ca')
            <div class="page" style="font-size: large;line-height: 2em">
                ...
                <div class="container" style="width:95%">
                    ...
                    @include('pdf.fct.partials.llistaAlumnes')
                    <p>Que dins d'aquesta empresa, En/Na/N' <strong>{{$instructor->nombre}} </strong> amb
                        DNI {{$instructor->dni}}, ha col.laborat en l'instrucció dels alumnes en les pràctiques
        ...
        @if (!isset($empresa->idioma) || $empresa->idioma == 'es')
            <div class="page" style="font-size: large;line-height: 2em">
                ...
                <div class="container" style="width:95%">
                    ...
                    @include('pdf.fct.partials.llistaAlumnes')
                    <p>Que dentro de esta empresa, Don/Doña <strong>{{$instructor->nombre}}</strong> con
                        DNI {{$instructor->dni}}, ha colaborador en la instrucción de los alumnos en las prácticas
