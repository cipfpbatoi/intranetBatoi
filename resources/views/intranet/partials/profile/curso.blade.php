    ...
    <div id="{{$elemento->id}}" class="well profile_view">
        <div class="col-sm-12">
            <h4 class="brief">
                ...
                @if ($elemento->horas > 0)
                <em class="fa fa-hourglass"></em> {{$elemento->horas}} {{ __('messages.generic.horas')}}
                @endif
        ...
        <div class="col-xs-12 bottom text-center">
            <div class="col-xs-12 col-sm-4 emphasis">
                <p class="ratings">
                    @if ($elemento->aforo == 0)
                        <a href='#' class='btn btn-success btn-xs' >{{ __('messages.buttons.disponible') }}</a>
                    @else
                        @if ($elemento->aforo > $elemento->NAlumnos)
                            <a href='#' class='btn btn-success btn-xs'>{{ __('messages.buttons.disponible') }}</a>
                        @else
                            <a href='#' class='btn btn-danger btn-xs'>{{ __('messages.buttons.nodisponible') }}</a>
                        @endif
                    @endif
                    @if ($elemento->Registrado())
                        <a href='#' class='btn btn-success btn-xs' >
                            {{ __('messages.buttons.registered') }}
                        </a>
                    @endif
                </p>
            </div>
