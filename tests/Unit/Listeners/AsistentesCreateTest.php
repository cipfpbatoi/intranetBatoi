...
class AsistentesCreateTest extends TestCase
{
    ...
    public function test_assigna_alumnes_en_avaluacio_final_de_primer_semipresencial(): void
    {
        DB::table('alumnos')->insert(['nia' => '10002395']);
        DB::table('reuniones')->insert([
            'id' => 2,
            'tipo' => 7,
            'numero' => 34,
        ...
        $this->assertSame(1, DB::table('alumno_reuniones')
            ->where('idReunion', 2)
            ->where('idAlumno', '10002395')
            ->where('capacitats', 3)
            ->count());
    }

    public function test_no_assigna_alumnes_en_avaluacio_final_de_segon_semipresencial(): void
    {
        DB::table('alumnos')->insert(['nia' => '10002396']);
        DB::table('reuniones')->insert([
            'id' => 3,
            'tipo' => 7,
            'numero' => 34,
        ...
        $grupoService = $this->createMock(GrupoService::class);
        $grupoService->method('largestByTutor')->with('P300')->willReturn($grupo);
        app()->instance(GrupoService::class, $grupoService);
