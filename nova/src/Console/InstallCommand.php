...
class InstallCommand extends Command
{
    ...
    public function handle()
    {
        ...
        $this->comment('Generating Profesor Resource...');
        $this->callSilent('nova:resource', ['name' => 'Profesor']);
        copy($this->resolveStubPath('/stubs/nova/user-resource.stub'), app_path('Nova/Profesor.php'));

        if (file_exists(app_path('Models/Profesor.php'))) {
            file_put_contents(
                app_path('Nova/Profesor.php'),
                str_replace('App\User::class', 'App\Models\User::class', file_get_contents(app_path('Nova/Profesor.php')))
            );
        }
    ...
    protected function setAppNamespace()
    {
        ...
        $this->setAppNamespaceOn(app_path('Nova/Profesor.php'), $namespace);
